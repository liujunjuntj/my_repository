...
<html lang="en">
	...
	<div class="container">
		<div style="font-size:30px;margin:20px;color:green">用户使用情况</div>
		<table class="table table-hover table-condensed table-bordered table-striped">
			<thead>
				<tr>
					<th width="10%">用户</th>
					<th width="45%">API-<span style="color:#FF5151">(<?php echo ($totalApi); ?>)</span></th>
					<th width="45%">测试用例-<span style="color:#FF5151">(<?php echo ($totalCase); ?>)</span></th>
				</tr>
			</thead>
			<tbody>
				<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><a href="<?php echo U('Api/alist');?>?owner=<?php echo ($vo["username"]); ?>"><?php echo ($vo["apiCount"]); ?></a></td>
					<td><a href="<?php echo U('Case/clist');?>?owner=<?php echo ($vo["username"]); ?>"><?php echo ($vo["caseCount"]); ?></a></td>
