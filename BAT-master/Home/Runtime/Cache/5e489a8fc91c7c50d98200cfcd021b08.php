...
<html lang="en">
	...
	<div class="span12">
		<ul class="breadcrumb">
			...
			<li><a href="<?php echo U('App/applist');?>">APP管理</a><span class="divider">/</span></li>
			<li>列表</li>
		</ul>
	</div>
	<div class="span12">
		<div class="bc">
			<form class="form-inline" action="<?php echo U('App/applist');?>" method="get">
				<label>ID：</label>
				<input style="width:100px;" type="text" name="id" placeholder="App--ID" value="<?php echo ($_GET['id']); ?>" /> 
				<label>Name：</label>
				<input class="span3" type="text" name="appName" placeholder="App--Name" value="<?php echo ($_GET['appName']); ?>"/> 
	...
	<?php if(getRole() == 1): ?><div class="btn-group tool-btn">
			<a href="<?php echo U('App/add');?>" id="app_add" class="btn btn-info">新增</a>
	...
	<div class="span12" id="apps-data">
		<table class="table table-hover table-condensed table-bordered table-striped">
			...
			<tbody>
				<?php if(!empty($apps)): if(is_array($apps)): $i = 0; $__LIST__ = $apps;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$app): $mod = ($i % 2 );++$i;?><tr>
					<?php if(getRole() == 1): ?><td style="text-align: center;"><input class="selectCell"
							type="checkbox" value="<?php echo ($app["id"]); ?>" /></td><?php endif; ?>
					<td><?php echo ($app["id"]); ?></td>	
					<td><?php echo ($app["appName"]); ?></td>
					<td title="<?php echo ($app["appDesc"]); ?>">
						<?php if(mb_strlen($app['appDesc'],'utf8') > 40): echo (mb_substr($app["appDesc"],0,40,utf8)); ?>...<?php else: echo ($app["appDesc"]); endif; ?>
					</td>	
					<td><?php echo ($app["createTime"]); ?></td>
					<td><?php echo ($app["updateTime"]); ?></td>
